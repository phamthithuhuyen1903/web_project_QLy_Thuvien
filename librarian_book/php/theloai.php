...
if (isset($_POST['action']) && $_POST['action'] == "delete") {
    ...
    // Kiểm tra xem loại sách này có sách nào không
    $check_sql = "SELECT COUNT(*) as total FROM sach WHERE ma_loai_sach = ?";
    $check_stmt = mysqli_prepare($conn, $check_sql);
    mysqli_stmt_bind_param($check_stmt, "s", $ma);
    mysqli_stmt_execute($check_stmt);
    $check_res = mysqli_stmt_get_result($check_stmt);
    $row = mysqli_fetch_assoc($check_res);

    if ($row['total'] > 0) {
        // Nếu có sách thuộc loại này thì không cho xóa
        echo "has_books";
    } else {
        // Nếu không có sách thì mới xóa
...
<head>
    ...
    <link rel="stylesheet" href="/Project_QuanLyThuVien/librarian_book/css/theloai.css">
...
<body>
    ...
    <div class="thanhdieuhuong">
        ...
        <a href="/Project_QuanLyThuVien/librarian_book/php/theloai.php" class="thanhdieuhuong_btn active">
