<?php
session_start();
require_once __DIR__ . '/../../connect/connect.php'; // Đảm bảo file này đã chuyển sang mysqli_connect
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($account) {
        ...
        if ($role == 0) {
            ...
            if ($profile) {
                $_SESSION['id'] = $profile['ma_admin'];
                $_SESSION['username'] = $profile['ho_ten'];
                $_SESSION['role'] = 'admin';
            } else {
                $_SESSION['id'] = $id_link;
                $_SESSION['username'] = "Admin (Chưa cập nhật tên)";
                $_SESSION['role'] = 'admin';
            }
        } else {
            ...
            $sqlProfile = "SELECT * FROM sinh_vien WHERE id = ?";
            $stmtProfile = mysqli_prepare($conn, $sqlProfile);
            mysqli_stmt_bind_param($stmtProfile, "s", $id_link);
            mysqli_stmt_execute($stmtProfile);
            $resultProfile = mysqli_stmt_get_result($stmtProfile);
            $profile = mysqli_fetch_assoc($resultProfile);
            // $sqlProfile = "SELECT * FROM sinh_vien WHERE ma_sv = '$id_link'";
            // $resultProfile = mysqli_query($conn, $sqlProfile);
            // var_dump(mysqli_fetch_assoc($resultProfile));
            // die();
            ...
            if ($profile) {
                ...
                $_SESSION['id'] = $id_link;
                $_SESSION['username'] = "Sinh viên (Chưa cập nhật tên)";
                $_SESSION['role'] = 'sinh_vien';
            }
        }
